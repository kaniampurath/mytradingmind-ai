...
def test_application_version_is_current_release() -> None:
    ...
    assert 'version = "1.2.9"' in pyproject
...
def test_release_docs_describe_baseline_and_current_tags() -> None:
    ...
    assert "`v1.2.9`: current main release" in readme
    assert "git checkout v1.0" in releases
    assert "git checkout v1.2.9" in releases
    assert "git checkout v1.2.9" in ubuntu
