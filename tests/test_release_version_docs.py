...
def test_application_version_is_v1_2_release() -> None:
...
def test_release_docs_describe_baseline_and_current_tags() -> None:
    ...
    assert "`v1.2`: current main release" in readme
    assert "git checkout v1.0" in releases
    assert "git checkout v1.2" in releases
    assert "git checkout v1.2" in ubuntu
