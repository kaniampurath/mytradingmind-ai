...
This guide installs mytradingmind.ai v1.2.6 on an Ubuntu droplet using Docker Compose, MariaDB 10.11, Redis 7, a headless bot runtime, scanner, and Streamlit operator dashboard.
...
Use the current release tag for a stable deployment. `v1.0` is the preserved baseline; `v1.2.6` is the current main release.
...
```bash
bash setup.sh --upgrade --target-version v1.2.6
```
