...
- `v1.2.7`: current main release with Bot Management, runtime cockpit, Trade Management, journal analysis, headless runtime scripts, Ubuntu install/upgrade hardening, Docker runtime entrypoint packaging, dashboard scan-file resilience, runtime status merge, security/RBAC foundations, AAPIF strategy evolution, and persisted bot trade-state recovery.
...
git checkout v1.2.7 # current main release
```
